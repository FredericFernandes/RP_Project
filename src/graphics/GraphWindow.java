...
public class GraphWindow extends SingleGraph{

	public GraphWindow(HashMap<Integer, SimulatedCity> listSimulatedCity, float[][] matriceWeight, boolean chowRoad, float[][] matriceRes) {
		super("Simulation");
	...
	private void drowResult(float[][] matriceRes, HashMap<Integer,SimulatedCity> listSimulatedCity) 
	{
		...
		for(int i =0 ; i< matriceRes.length ; i++){
			if(matriceRes[i][i]!=0){		
				...
				Node n = getNode(id);
				n.addAttribute("ui.style", "size: 10px, 10px; fill-color: green;");
